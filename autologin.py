...
def in_bphc_network():
    response = os.system("ping -c 1 172.16.0.30 > /dev/null 2>&1")
    if response == 0:
        return True
    else:
        return False
...
def net_accessible(retry_count=5):
    if not retry_count:
        ...
        return False
    if not in_bphc_network():
        logging.critical("Not in BPHC network, Quitting!")
        exit(3)
    try:
...
def main():
    ...
    while True:
        if int(used_data) > 4500:
            logging.warning("Data finished for this user")
